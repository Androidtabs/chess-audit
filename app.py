...
if not imgs:
    ...
    # Usamos uma coluna central para a imagem
    _, col_img, _ = st.columns([1, 4, 1])
    with col_img:
        st.image(path_img, use_container_width=True)

    # --- BOTÕES ABAIXO (CENTRALIZADOS) ---
    # Usamos 4 colunas: as do meio (b1, b2) ficam coladas no centro
    _, b1, b2, _ = st.columns([4.5, 0.5, 0.5, 4.5])
    with b1:
